...
|---|---|---|---|
| Grocy | 바코드 입고, best-before, 개봉, 냉동·해동, 폐기, recipe API | HTTP adapter·system status·상품/단위/location mapping·상품 mapping before→after audit·receipt/storage-event outbox·retry/dead-letter·in-flight reconciliation·workspace lease/worker heartbeat contract·disposable 4.7.0 add/open/consume/transfer readback | 운영 버전·backup/restore·API key rotation·중복 mutation·crash recovery·undo/보상 동작 |
...
| Open Food Facts v3.6 | 바코드로 상품 조회 | 한국 상품 후보·카테고리·용량 enrichment, User-Agent·bounded cache·429/503 rate-limit 상태 | 한국 제품 coverage·정확성·실서비스 quota |
| Open Food Facts legacy search | 축약 상품명 텍스트 검색 | I1250 miss 뒤 명시적 review/enrichment fallback, 검색 전용 rate-limit·provider별 cache-key·source provenance | 최신 v3 full-text search 부재·사용자 기여 데이터·검색 quota·autocomplete 금지 |
| 식품안전나라 `I1250` | 제품명·품목제조보고 조회 | opt-in 제품명 review endpoint·제조사·품목유형·`POG_DAYCNT` 후보·receipt 비동기 enrichment worker·Open Food Facts fallback | API 인증키·제품 수준 정보·개별 lot 아님·실제 quota/coverage |
| 식품안전나라 `C005` | 바코드연계제품 조회 | opt-in adapter·제품 기준 `POG_DAYCNT`·명시적 보관 힌트·legacy provenance | 2018년 이후 최신화 중단 안내·API key·개별 lot 날짜 아님 |
| pypdf | PDF file-like stream·page text extraction 재현 | text-layer 전자 영수증을 기존 receipt parser/review draft로 연결, bbox 없는 보수적 경로 | scan PDF·layout/table·source bbox는 별도 처리 |
| pypdfium2 | PDFium 기반 bounded page render 재현 | scan PDF page를 PNG로 변환해 기존 OCR adapter에 연결, 최대 3쪽·scale 2 quota | 실제 대용량/악성 PDF resource budget·page-aware bbox |
| Docling | PDF·문서 layout/표/reading order 재현 | multi-page 전자 영수증·주문확인서 구조화 후속 후보 | 포장 라벨 실시간 OCR에는 과할 수 있음 |
...
| Ollama | JSON Schema structured output 재현 | 규칙 미매칭 상품명 priority fallback·Pydantic 검증·abstain·review-only provenance | 실제 모델 정확도·domain benchmark·구조 준수는 사실성 증거가 아님 |
...
| OR-Tools | 제약식 기반 scheduling·CP-SAT 예제 | 이미 materialize한 recipe 후보 중 lot capacity·recipe 중복·연속 3일을 함께 선택 | solver version/latency benchmark·영양/예산 확장 |
...
```text
image → image quality gate → PaddleOCR text boxes
text-layer PDF → pypdf page text
scan PDF → pypdfium2 bounded PNG pages → PaddleOCR text
