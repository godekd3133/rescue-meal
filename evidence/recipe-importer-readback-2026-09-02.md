...
선택적 식품안전나라 `COOKRCP01` importer의 source adapter, review draft 변환, bad row 격리, API key 비노출, 운영 CLI를 코드 수준에서 검증했습니다. 현재 실행환경에는 `FOODSAFETY_COOKRCP_API_KEY`가 없으므로 실제 공공 API에 요청을 보내거나 운영 이용조건을 승인한 결과는 아닙니다.
...
apps/web: connected E2E                             → 2 passed
...
- 현재 importer는 외부 draft를 deterministic planner fixture로 자동 승격하지 않습니다.
