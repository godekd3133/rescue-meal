...
- Contract snapshot 당시 API 전체 테스트: `54 passed, 2 warnings`; 이후 레시피 preview/save/latest와 조리 완료·multi-lot·사용량 조정·단위 환산·audit 회귀를 포함한 현재 전체 API suite는 `70 passed, 3 warnings`입니다.
...
Docker daemon이 꺼져 있어 실제 Grocy container, product ID mapping, stock mutation, transaction ID/undo, timeout·retry·reconciliation readback은 아직 실행하지 않았습니다. 현재 UI의 영수증 commit과 storage event는 Grocy에 자동 write하지 않습니다.
