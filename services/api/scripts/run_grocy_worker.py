...
from app.grocy_worker import GrocyWorkerSettings
from app.worker_runner import request_http_tick, run_worker_loop


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rescue Meal Grocy outbox worker")
    parser.add_argument(
        "--once",
        action="store_true",
...
def _disabled_result(settings: GrocyWorkerSettings) -> list[dict[str, object]]:
    return [
        {
            "workspace_id": workspace_id,
            "worker_id": settings.worker_id,
            "status": "disabled",
            "reason": "RESCUE_MEAL_GROCY_WORKER_TOKEN이 없어 API tick을 호출하지 않았습니다.",
        }
...
def run(*, once: bool = False, sleep=time.sleep, emit=None) -> int:
    ...
    worker_token = os.getenv("RESCUE_MEAL_GROCY_WORKER_TOKEN", "").strip()
    if not worker_token:
    ...
    with httpx.Client(base_url=api_base_url, timeout=10.0) as client:
        def tick(workspace_id: str) -> tuple[dict[str, object], bool]:
            return request_http_tick(
                client,
                path=f"/api/internal/grocy/workspaces/{quote(workspace_id, safe='')}/tick",
                headers={"X-Rescue-Meal-Grocy-Worker-Token": worker_token},
                payload={
                    "worker_id": settings.worker_id,
