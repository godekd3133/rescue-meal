      ...
      {loading && !preferences ? <div className="account-loading" role="status">알림 설정을 불러오고 있어요</div> : (
        <>
          ...
          <div className="push-subscription-card">
            <div className="push-subscription-heading"><span><strong>Web Push</strong><small>브라우저를 닫아도 확인할 알림을 받을 수 있어요.</small></span><span className={`receipt-privacy-badge ${subscriptions.length ? "receipt-privacy-badge-redacted" : ""}`}>{subscriptions.length ? `연결된 기기 ${subscriptions.length}개` : "연결된 기기 없음"}</span></div>
            <div className="notification-delivery-status"><span><strong>전달 worker</strong><small>{workerHeartbeat ? workerHeartbeat.push_configured ? workerHeartbeat.last_error ? `확인 필요 · ${workerHeartbeat.last_error}` : `정상 · ${formatWorkerTime(workerHeartbeat.last_tick_at)} 확인 · ${workerHeartbeat.succeeded}건 전달${workerHeartbeat.cancelled ? ` · ${workerHeartbeat.cancelled}건 읽음 후 취소` : ""}` : "VAPID 설정이 없어 전달이 비활성화되어 있어요." : "notification worker가 아직 실행되지 않았어요."}</small></span><span className={`notification-delivery-badge ${workerHeartbeat?.push_configured && !workerHeartbeat.last_error ? "notification-delivery-badge-ready" : ""}`}>{workerHeartbeat?.push_configured && !workerHeartbeat.last_error ? "준비됨" : "대기"}</span></div>
...
function PasswordChangePanel() {
  ...
  return (
    <section className="password-change-panel" aria-label="비밀번호 변경">
      {!open ? <button className="account-security-button" type="button" onPointerDown={(event) => event.preventDefault()} onClick={() => { keyboard.hide(); setError(""); setNotice(""); setOpen(true); }}><span><strong>비밀번호 변경</strong><small>변경하면 기존 account session을 모두 다시 확인해요.</small></span><ArrowRightIcon width={15} height={15} /></button> : (
        <>
          <div className="password-change-heading"><div><h3 id="password-change-title">비밀번호 변경</h3><p>새 비밀번호를 저장하면 기존 account token은 바로 만료돼요.</p></div><button className="grocy-refresh-button" type="button" onClick={() => { keyboard.hide(); setOpen(false); setError(""); }}>닫기</button></div>
...
function AccountDeletionPanel({ onDeleted }: { onDeleted: () => void | Promise<void> }) {
  ...
  return (
    <section className="account-delete-panel" aria-label="계정 삭제">
      {!open ? <button className="account-danger-button" type="button" onPointerDown={(event) => event.preventDefault()} onClick={() => { keyboard.hide(); setError(""); setRetryableFailure(false); setOpen(true); }}><span><strong>계정 삭제</strong><small>계정과 이 workspace의 기록을 영구적으로 삭제해요.</small></span><ArrowRightIcon width={15} height={15} /></button> : (
        <>
          ...
          <p className="account-delete-warning"><InfoCircledIcon width={14} height={14} /> 서버 backup·외부 Grocy 보존 정책은 별도일 수 있어요. 삭제 요청 후에는 다시 로그인할 수 없습니다.</p>
        </>
      )}
    </section>
  );
...
function PasswordRecoveryPanel({ initialToken, onAuthenticated }: { initialToken?: string; onAuthenticated: (session: ApiAuthSession) => void | Promise<void> }) {
  const keyboard = useKeyboard();
      ...
      {!open ? <button className="account-security-button" type="button" onPointerDown={(event) => event.preventDefault()} onClick={() => { keyboard.hide(); setError(""); setNotice(""); setOpen(true); }}><span><strong>비밀번호를 잊으셨나요?</strong><small>등록된 이메일로 재설정 안내를 요청해요.</small></span><ArrowRightIcon width={15} height={15} /></button> : initialToken ? (
        <>
          ...
          <div className="password-change-form">
            ...
            <KeyboardInput id="reset-new-password-input" className="app-input" type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} onBlur={() => keyboard.hide()} />
            <label className="app-input-label" htmlFor="reset-confirm-password-input">새 비밀번호 확인</label>
            <KeyboardInput id="reset-confirm-password-input" className="app-input" type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} onBlur={() => keyboard.hide()} />
            {error ? <div className="account-error" role="alert"><InfoCircledIcon width={15} height={15} /><span>{error}</span></div> : null}
        ...
        <>
          ...
          <div className="password-change-form">
            ...
            <KeyboardInput id="reset-email-input" className="app-input" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} onBlur={() => keyboard.hide()} />
            {error ? <div className="account-error" role="alert"><InfoCircledIcon width={15} height={15} /><span>{error}</span></div> : null}
    ...
    try {
      ...
      if (isMealApiConflictError(reason) && onConflict) {
        try {
          ...
          setError("계정 workspace가 변경됐어요. 최신 게스트 기록을 다시 확인해 주세요.");
          return;
        }
      }
  ...
  return (
    <section className="guest-transfer-panel" aria-labelledby="guest-transfer-title">
      <div className="guest-transfer-heading">
        ...
        <div><h3 id="guest-transfer-title">게스트 기록을 가져올까요?</h3><p>방금까지 이 기기에서 기록한 내용을 계정 workspace로 복사해요.</p></div>
      </div>
      <div className="guest-transfer-summary" role="status">
        <strong>가져올 기록</strong>
        <span>식품 {preview.food_count}개 · 영수증 {preview.receipt_count}개 · 보관 기록 {preview.storage_event_count}개 · 식단 {preview.meal_plan_count}개{preview.multi_day_plan_count ? ` · 3일 계획 ${preview.multi_day_plan_count}개` : ""}{preview.shopping_list_count ? ` · 장보기 ${preview.shopping_list_count}개` : ""}{preview.shopping_receive_operation_count ? ` · 입고 확인 ${preview.shopping_receive_operation_count}개` : ""}{preview.meal_preferences_changed ? " · 식단 조건" : ""}{preview.notification_preferences_changed ? " · 알림 설정" : ""}</span>
      </div>
      <p className="guest-transfer-note"><InfoCircledIcon width={14} height={14} /> 게스트 workspace는 삭제하지 않아요. 계정에 이미 기록이 있으면 자동으로 섞지 않고 확인을 요청해요.</p>
      {error ? <div className="account-error" role="alert"><InfoCircledIcon width={15} height={15} /><span>{error}</span></div> : null}
      <div className="guest-transfer-actions">
        <button className="primary-sheet-button" type="button" disabled={busy} onPointerDown={(event) => event.preventDefault()} onClick={() => void submit(onImport)}>{busy ? "처리 중이에요" : "게스트 기록 가져오기"}<ArrowRightIcon width={15} height={15} /></button>
  ...
  return (
    <section className="guest-transfer-panel guest-transfer-panel-warning" aria-labelledby="guest-transfer-retry-title">
      <div className="guest-transfer-heading">
        ...
        <div><h3 id="guest-transfer-retry-title">게스트 기록을 아직 확인하지 못했어요</h3><p>{message}</p></div>
      </div>
      <p className="guest-transfer-note"><InfoCircledIcon width={14} height={14} /> 게스트 workspace의 기록은 삭제되지 않아요. 지금 건너뛰어도 같은 계정으로 다시 들어오면 이어서 가져올 수 있어요.</p>
      {error ? <div className="account-error" role="alert"><InfoCircledIcon width={15} height={15} /><span>{error}</span></div> : null}
      <div className="guest-transfer-actions">
        <button className="primary-sheet-button" type="button" disabled={busy} onPointerDown={(event) => event.preventDefault()} onClick={() => void submit(onRetry)}>{busy ? "확인 중이에요" : "다시 확인"}<ArrowRightIcon width={15} height={15} /></button>
        <button className="secondary-sheet-button" type="button" disabled={busy} onPointerDown={(event) => event.preventDefault()} onClick={() => void submit(onSkip)}>계정만 사용</button>
      </div>
    </section>
  );
...
function WorkspaceDataExportPanel({ workspaceSync }: { workspaceSync: WorkspaceSyncCoordinator }) {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      setNotice("내 workspace 데이터를 내보냈어요");
    } catch (reason) {
  ...
  return (
    <section className="workspace-export-panel" aria-labelledby="workspace-export-title">
      <div className="workspace-export-heading"><span className="workspace-export-icon"><ReaderIcon width={17} height={17} /></span><div><h3 id="workspace-export-title">내 데이터 내보내기</h3><p>다른 곳에 보관할 수 있는 JSON 사본을 만들어요.</p></div></div>
      <div className="workspace-export-note"><InfoCircledIcon width={15} height={15} /><span>재고·사용자 정의 보관 위치·구매 summary·보관 기록·식단·입고 중복 방지 기록·알림 설정을 포함하고, 비밀번호·access token·원본 OCR·push endpoint는 포함하지 않아요.</span></div>
      {error ? <div className="account-error" role="alert"><InfoCircledIcon width={15} height={15} /><span>{error}</span></div> : null}
      {notice ? <div className="grocy-success" role="status"><CheckCircledIcon width={15} height={15} /><span>{notice}</span></div> : null}
      <button className="secondary-sheet-button workspace-export-button" type="button" disabled={busy} onPointerDown={(event) => event.preventDefault()} onClick={() => { keyboard.hide(); void exportData(); }}>{busy ? "파일을 준비하는 중" : "내 데이터 JSON 다운로드"}<ArrowRightIcon width={15} height={15} /></button>
    </section>
  );
}
...
function GrocyIntegrationPanel({ workspaceSync, refreshNonce }: { workspaceSync: WorkspaceSyncCoordinator; refreshNonce: number }) {
  const keyboard = useKeyboard();
  ...
  const refresh = async (initial = false) => {
    ...
    if (!result.current) return;
    if (result.error || !result.value) {
      setError("Grocy 설정을 불러오지 못했어요. 서버 연결 상태를 확인해 주세요.");
      setRetryAction({ kind: "refresh" });
      setLoading(false);
      setRefreshing(false);
      return;
  ...
  const saveLocation = async (storageType: ApiStorageType) => {
    ...
    if (!Number.isInteger(value) || value <= 0) {
      setError("Grocy location ID는 1 이상의 숫자로 입력해 주세요.");
      return;
    }
    ...
    try {
      ...
      if (isMealApiWorkspaceConflictError(reason)) {
        ...
        setError(isMealApiGrocyLocationMappingPersistenceError(reason)
          ? "보관 위치 매핑을 저장하지 못했어요. 기존 매핑과 outbox 상태를 유지했어요."
          : "보관 위치 매핑을 저장하지 못했어요.");
  ...
  const saveProductMapping = async (canonicalName: string, draftKey: string, fallbackUnit: string) => {
    ...
    if (!Number.isInteger(productId) || productId <= 0 || !unit) {
      setError("Grocy product ID와 Rescue Meal 단위를 모두 입력해 주세요.");
      return;
    }
    ...
    try {
      ...
      if (isMealApiWorkspaceConflictError(reason)) {
        ...
        setError(isMealApiGrocyMappingPersistenceError(reason)
          ? "상품 매핑을 저장하지 못했어요. 기존 매핑과 outbox 상태를 유지했어요."
          : "상품 매핑을 저장하지 못했어요.");
  ...
  const processOutbox = async () => {
    ...
    try {
      ...
      setNotice(`Grocy 동기화 ${result.succeeded}건을 완료하고 ${result.retried}건을 재시도 대기했어요.`);
      await refresh();
    } catch (reason) {
      setError(isMealApiWorkspaceConflictError(reason)
        ? MEAL_API_WORKSPACE_CONFLICT_MESSAGE
        : "Grocy 동기화를 실행하지 못했어요. 연결 상태와 매핑을 확인해 주세요.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const scanReconciliation = async () => {
    if (!inFlightOutbox.length) return;
    setScanning(true);
    setError("");
    try {
  ...
  const reconcileOutbox = async (record: ApiGrocyOutboxRecord, decision: "already_applied" | "not_applied") => {
    ...
    if (decision === "already_applied" && !transactionId) {
      setError("반영됨으로 처리하려면 Grocy transaction ID를 입력해 주세요.");
      return;
    }
    ...
    try {
      await mealApi.reconcileGrocyOutbox(record.id, {
        ...
        operator_note: decision === "already_applied" ? "운영자가 Grocy 반영을 확인" : "운영자가 미반영을 확인",
      });
  ...
  const retryOutbox = async (record: ApiGrocyOutboxRecord) => {
    ...
    try {
      ...
      if (isMealApiWorkspaceConflictError(reason)) {
        ...
        setError(isMealApiGrocyOutboxPersistenceError(reason)
          ? "실패한 Grocy 작업을 재시도 대기로 돌리지 못했어요. 기존 상태를 유지했어요."
          : "실패한 Grocy 작업을 재시도 대기로 돌리지 못했어요.");
  ...
  return (
    <section className="grocy-integration" aria-labelledby="grocy-integration-title">
      <div className="grocy-integration-heading">
        <span className="grocy-integration-icon"><SewingPinIcon width={17} height={17} /></span>
        <div>
          <h3 id="grocy-integration-title">Grocy 재고 연동</h3>
          <p>입고·먹음·폐기·보관 이동을 내 workspace와 함께 기록해요.</p>
        </div>
        <button className="grocy-refresh-button" type="button" aria-label="Grocy 설정 새로고침" disabled={loading || refreshing} onPointerDown={(event) => { event.preventDefault(); keyboard.hide(); void refresh(); }} onClick={(event) => { if (event.detail === 0) void refresh(); }}>
      ...
      <div className={`grocy-status-card grocy-status-${status?.status ?? "loading"}`} data-status={status?.status ?? "loading"}>
        ...
        <div><strong>{grocyStatusLabel(status?.status)}{status?.version ? ` · v${status.version}` : ""}</strong><small>{status?.detail ?? (loading ? "서버 설정을 확인하고 있어요." : "상태를 읽지 못했어요.")}</small></div>
      </div>
      ...
      <div className={`grocy-worker-card ${workerHeartbeat?.last_error ? "grocy-worker-error" : ""}`} data-worker-state={workerHeartbeat ? workerHeartbeat.last_error ? "error" : "ready" : "unknown"}>
        <div><strong>자동 동기화 worker</strong><small>{status?.configured !== true ? "Grocy 서버 설정 후 자동 처리가 시작됩니다." : workerHeartbeat ? workerHeartbeat.last_error ? `마지막 실행 오류: ${workerHeartbeat.last_error}` : `마지막 확인 ${formatWorkerTime(workerHeartbeat.last_tick_at)} · ${workerHeartbeat.processed}건 처리` : "worker heartbeat를 아직 받지 못했어요."}</small></div>
        <span>{workerHeartbeat?.last_error ? "확인 필요" : workerHeartbeat ? "정상" : "대기 중"}</span>
      </div>

      {error ? <div className="account-error grocy-error" role="alert"><InfoCircledIcon width={16} height={16} /><span>{error}</span>{retryAction ? <button className="account-error-action" type="button" onClick={retry}>{retryAction.kind === "refresh" ? "최신 상태 확인" : "다시 시도"}</button> : null}</div> : null}
      {notice ? <div className="grocy-success" role="status"><CheckCircledIcon width={15} height={15} /><span>{notice}</span></div> : null}

      <div className="grocy-settings-block">
        <div className="grocy-block-heading"><strong>보관 위치 연결</strong><small>Grocy에서 사용하는 location ID를 확인해 주세요.</small></div>
          ...
          {STORAGE_OPTIONS.map((option) => {
            ...
            return (
              <div className="grocy-location-row" key={option.value}>
                ...
                <KeyboardInput className="grocy-number-input" type="number" inputMode="numeric" min={1} value={value} placeholder="예: 20" aria-label={`${option.label} Grocy location ID`} disabled={status?.configured !== true} onChange={(event) => setLocationDrafts((current) => ({ ...current, [option.value]: event.target.value }))} onBlur={() => keyboard.hide()} />
        ...
        {status?.configured !== true ? <p className="grocy-inline-note"><InfoCircledIcon width={14} height={14} />Grocy URL과 API key는 서버 운영자가 설정해야 이 화면에서 매핑을 저장할 수 있어요.</p> : null}
      </div>

      {productTasks.length ? (
        <div className="grocy-settings-block">
          <div className="grocy-block-heading"><strong>확인이 필요한 상품</strong><small>영수증·이벤트가 멈춘 상품만 표시해요.</small></div>
          <div className="grocy-product-task-list">
            {productTasks.map((record) => {
              ...
              return (
                <div className="grocy-product-task" key={key}>
                  ...
                  <div className="grocy-product-task-fields">
                    <KeyboardInput className="grocy-number-input" type="number" inputMode="numeric" min={1} value={draft.productId} placeholder="product ID" aria-label={`${record.canonical_name} Grocy product ID`} disabled={status?.configured !== true || busy} onChange={(event) => setProductDrafts((current) => ({ ...current, [key]: { ...draft, productId: event.target.value } }))} onBlur={() => keyboard.hide()} />
                    <KeyboardInput className="grocy-unit-input" value={draft.unit} placeholder={record.unit} aria-label={`${record.canonical_name} Grocy 단위`} disabled={status?.configured !== true || busy} onChange={(event) => setProductDrafts((current) => ({ ...current, [key]: { ...draft, unit: event.target.value } }))} onBlur={() => keyboard.hide()} />
      ...
      {productMappings.length ? (
        <div className="grocy-settings-block">
          <div className="grocy-block-heading"><strong>등록된 상품 연결</strong><small>저장된 Grocy product ID와 단위를 검토하거나 수정해요.</small></div>
          <KeyboardInput className="grocy-mapping-search" value={productQuery} placeholder="상품명 검색" aria-label="등록된 Grocy 상품 매핑 검색" onChange={(event) => setProductQuery(event.target.value)} onBlur={() => keyboard.hide()} />
                ...
                <div className="grocy-product-catalog-item" key={key}>
                  <div className={`grocy-product-catalog-row ${editing ? "grocy-product-catalog-editing" : ""}`}>
                    {editing ? (
                      <>
                        <div className="grocy-product-task-fields">
                          <KeyboardInput className="grocy-number-input" type="number" inputMode="numeric" min={1} value={draft.productId} aria-label={`${mapping.canonical_name} 기존 Grocy product ID`} disabled={busy} onChange={(event) => setProductDrafts((current) => ({ ...current, [key]: { ...draft, productId: event.target.value } }))} onBlur={() => keyboard.hide()} />
                          <KeyboardInput className="grocy-unit-input" value={draft.unit} aria-label={`${mapping.canonical_name} 기존 Grocy 단위`} disabled={busy} onChange={(event) => setProductDrafts((current) => ({ ...current, [key]: { ...draft, unit: event.target.value } }))} onBlur={() => keyboard.hide()} />
                      ...
                      <>
                        <div className="grocy-product-catalog-copy"><strong>{mapping.canonical_name}</strong><small>Grocy #{mapping.grocy_product_id} · {mapping.grocy_unit ?? "단위 미확인"} · {mapping.updated_by_email ?? mapping.updated_by ?? "변경 주체 기록 없음"} · {formatMappingTime(mapping.updated_at)}</small></div>
                  ...
                  {mappingHistoryKey === key ? (
                    <div className="grocy-product-history" role="region" aria-label={`${mapping.canonical_name} 매핑 변경 이력`}>
                      ...
                      {mappingEventsLoading === key ? <p className="grocy-inline-note">이력을 불러오는 중이에요.</p> : mappingEvents[key]?.length ? mappingEvents[key].map((event) => {
                        ...
                        const beforeProduct = event.before ? `Grocy #${event.before.grocy_product_id}` : "처음 연결";
                        ...
                        return (
                          <div className="grocy-product-history-event" key={event.id} data-history-action={event.action}>
                            ...
                            <small>{beforeProduct} · {beforeUnit} → Grocy #{event.after.grocy_product_id} · {event.after.grocy_unit ?? "단위 미설정"}</small>
                          </div>
                        );
            ...
            {reconciliationOutbox.map((record) => {
              ...
              return (
                <div className="grocy-reconciliation-row" key={record.id}>
                  <div className="grocy-reconciliation-copy"><strong>{record.canonical_name}</strong><small>{operationLabel(record)} · {record.last_error ?? "외부 반영 여부 확인 필요"}</small></div>
                  <div className="grocy-reconciliation-actions">
                    <KeyboardInput className="grocy-transaction-input" value={reconciliationDrafts[record.id] ?? ""} placeholder="transaction ID" aria-label={`${record.canonical_name} Grocy transaction ID`} disabled={busy} onChange={(event) => setReconciliationDrafts((current) => ({ ...current, [record.id]: event.target.value }))} onBlur={() => keyboard.hide()} />
                    <button className="grocy-save-button" type="button" disabled={busy} onPointerDown={(event) => { event.preventDefault(); keyboard.hide(); void reconcileOutbox(record, "already_applied"); }} onClick={(event) => { if (event.detail === 0) void reconcileOutbox(record, "already_applied"); }}>{busy ? "저장 중" : "반영됨"}</button>
                    <button className="grocy-retry-button" type="button" disabled={busy} onPointerDown={(event) => { event.preventDefault(); keyboard.hide(); void reconcileOutbox(record, "not_applied"); }} onClick={(event) => { if (event.detail === 0) void reconcileOutbox(record, "not_applied"); }}>미반영·재시도</button>
                  </div>
                </div>
              );
            })}
            ...
            {deadLetterOutbox.map((record) => {
              ...
              return (
                <div className="grocy-dead-letter-row" key={record.id}>
                  <div><strong>{record.canonical_name}</strong><small>{operationLabel(record)} · {record.last_error ?? record.last_dead_letter_error ?? "외부 동기화 실패"}</small></div>
      ...
      <div className="grocy-outbox-card">
        <div><strong>동기화 대기</strong><small>{blockedOutbox.length ? `매핑 필요 ${blockedOutbox.length}건` : "막힌 작업 없음"} · {pendingOutbox.length}건 처리 대기{inFlightOutbox.length ? ` · 처리 중 ${inFlightOutbox.length}건` : ""}{reconciliationOutbox.length ? ` · 확인 필요 ${reconciliationOutbox.length}건` : ""}{deadLetterOutbox.length ? ` · 실패 ${deadLetterOutbox.length}건` : ""}</small></div>
        ...
        <button className="secondary-sheet-button grocy-process-button" type="button" disabled={status?.configured !== true || !pendingOutbox.length || processing} onPointerDown={(event) => { event.preventDefault(); keyboard.hide(); void processOutbox(); }} onClick={(event) => { if (event.detail === 0) void processOutbox(); }}>{processing ? "동기화 중" : "지금 동기화"}<ArrowRightIcon width={15} height={15} /></button>
      </div>
    </section>
  );
}
...
export default function AccountSheet({
  ...
  if (authMe?.mode === "account" && authMe.email) {
    ...
    return (
      <div className="account-sheet-content">
        <div className="account-lead"><div className="capture-visual compact"><CheckCircledIcon width={23} height={23} /></div><div><h3>계정 workspace에 연결됨</h3><p>{authMe.email}의 식품 기록을 이어가고 있어요.</p></div></div>
        <div className="account-profile"><span className="account-profile-icon"><ReaderIcon width={18} height={18} /></span><span><strong>{authMe.email}</strong><small>{authMe.role === "recipe_admin" ? "recipe catalog 운영자 권한이 있어요." : "다른 기기에서도 같은 workspace를 열 수 있어요."}</small></span></div>
        {remoteRefreshNotice}
        ...
        {mealApi.isConfigured ? <StorageLocationPanel key={`storage-locations:${authMe.workspace_id}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
        {mealApi.isConfigured ? <NotificationPreferencesPanel key={`notification-preferences:${authMe.workspace_id}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
        {mealApi.isConfigured ? <WorkspaceDataExportPanel workspaceSync={workspaceSync} /> : null}
        {mealApi.isConfigured ? <ReceiptPrivacyPanel key={`receipt-privacy:${authMe.workspace_id}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
        {mealApi.isConfigured ? <GrocyIntegrationPanel key={`grocy:${authMe.workspace_id}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
        <button className="secondary-sheet-button" type="button" disabled={busy} onClick={() => void signOut()}>로그아웃</button>
        <div className="account-note"><InfoCircledIcon width={15} height={15} /><span>로그아웃하면 새 게스트 workspace로 전환됩니다. 현재 계정 기록은 삭제되지 않아요.</span></div>
      </div>
    );
  }
  ...
  return (
    <div className="account-sheet-content">
      <div className="account-lead"><div className="capture-visual compact"><ReaderIcon width={23} height={23} /></div><div><h3>내 식품을 안전하게 이어가기</h3><p>계정을 만들면 다른 기기에서도 같은 workspace를 다시 열 수 있어요.</p></div></div>
      {remoteRefreshNotice}
      <PasswordRecoveryPanel onAuthenticated={onAuthenticated} />
      {mealApi.isConfigured ? <StorageLocationPanel key={`storage-locations:${authMe?.workspace_id ?? "anonymous"}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
      {mealApi.isConfigured ? <NotificationPreferencesPanel key={`notification-preferences:${authMe?.workspace_id ?? "anonymous"}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
      {mealApi.isConfigured ? <WorkspaceDataExportPanel workspaceSync={workspaceSync} /> : null}
      {mealApi.isConfigured ? <ReceiptPrivacyPanel key={`receipt-privacy:${authMe?.workspace_id ?? "anonymous"}`} workspaceSync={workspaceSync} refreshNonce={panelRefreshNonce} /> : null}
