...
export default function FoodDetailSheet({
  ...
  const dateConfirmationAction = canConfirmDate
    ? readOnly
      ...
      : dateEditorOpen
        ? <Suspense fallback={<div className="date-editor-loading" role="status">날짜 입력 화면을 준비하고 있어요</div>}><DateAssertionEditor initialValue={dateEditorValue(food)} initialKind={dateEditorKind(food)} title={editingUserReminder ? "확인한 알림 날짜 수정" : undefined} description={editingUserReminder ? "기존 알림 날짜와 의미를 다시 확인해요." : undefined} onCancel={() => setDateEditorOpen(false)} onConfirm={(dateValue, kind) => onConfirmDate(food.id, dateValue, kind)} /></Suspense>
        : <button ref={dateConfirmationActionRef} className="date-edit-button" type="button" onClick={() => setDateEditorOpen(true)}><CalendarIcon width={15} height={15} /><span><strong>{editingUserReminder ? "확인한 알림 날짜 수정" : "포장지에서 확인한 날짜 입력"}</strong><small>{editingUserReminder ? "기존 알림 날짜와 의미를 다시 확인해요." : "확인 후 소비기한·품질유지기한·알림일로 저장해요."}</small></span><ArrowRightIcon width={15} height={15} /></button>
    : null;
  ...
  const detailActionsSection = (
    <div className="detail-actions-anchor" ref={detailActionsAnchorRef}>
      ...
      {readOnly ? <div className="detail-actions detail-actions-read-only" role="note"><InfoCircledIcon width={16} height={16} /><span><strong>기록 기능은 잠시 쉬고 있어요</strong><small>다시 연결하면 보관 상태 저장과 소비 기록을 이어갈 수 있어요.</small></span></div> : consumeConfirm ? <div ref={consumeConfirmRef} className="discard-confirm consume-confirm" role="alert"><div><strong>조리 전 확인이 필요해요</strong><small>포장지와 보관 상태를 확인한 뒤 먹은 기록을 남겨 주세요. Rescue Meal은 안전 여부를 판정하지 않아요.</small></div><div className="discard-confirm-actions"><button className="secondary-sheet-button" type="button" onClick={() => setConsumeConfirm(false)}>돌아가기</button><button className="primary-sheet-button" type="button" onClick={() => onConsume(food.id, eventQuantity)}>확인했어요 · 먹었어요 기록</button></div></div> : (
        <>
          <div className="detail-actions"><button ref={detailActionPrimaryRef} className="secondary-sheet-button" type="button" onClick={requestConsume}><CheckCircledIcon width={17} height={17} /> {eventQuantity < availableQuantity.amount ? `${eventQuantity}${availableQuantity.unit} 먹었어요` : "먹었어요"}</button><button className={`primary-sheet-button${hasPendingSaveChanges ? " detail-save-pending" : ""}`} style={!hasPendingSaveChanges ? { borderColor: "var(--atelier-border-strong)", background: "color-mix(in srgb, var(--atelier-surface-raised) 82%, transparent)", color: "var(--atelier-muted)", boxShadow: "none", opacity: 0.78 } : undefined} type="button" disabled={!hasPendingSaveChanges} title={!hasPendingSaveChanges ? detailSaveHint : undefined} onClick={() => onSave(food.id, storage, opened, eventQuantity, storageLocationId)}>{hasPendingSaveChanges ? <>변경 저장 <ArrowRightIcon width={17} height={17} /></> : "변경 없음"}</button></div>
          {!hasPendingSaveChanges ? <p className="detail-save-hint" role="note" style={{ display: "flex", gap: 6, alignItems: "flex-start", margin: "6px 2px 0", padding: 0, border: 0, background: "transparent", color: "var(--atelier-muted)", fontSize: 9, lineHeight: 1.45 }}><InfoCircledIcon width={14} height={14} />{detailSaveHint}</p> : null}
        </>
      )}
    </div>
  );
  ...
  return (
      ...
      <div className="detail-hero"><div className="detail-image-wrap"><img src={food.image} alt="" className="detail-image" draggable={false} /></div><div className="detail-hero-copy"><span className={`storage-pill ${getStorageClass(food.storage)}`}>{food.storageLocationName ?? food.storage} 보관 중</span><h3>{food.name}</h3><p>{food.brand} · 남은 {food.quantity}</p></div></div>
      ...
      {editProductInfoOpen ? <div className="product-info-editor" role="group" aria-label="상품 정보 수정"><div className="product-info-editor-heading"><span><Pencil1Icon width={16} height={16} /><strong>상품 정보 수정</strong></span><small>{productInfoSaving ? "서버에 저장 중이에요. 입력값은 유지돼요." : "상품명·브랜드·분류를 확인해요."}</small></div><label className="product-info-editor-field"><span>상품명</span><KeyboardInput className="app-input" value={productNameDraft} autoComplete="off" spellCheck={false} aria-label="상품명 수정" disabled={productInfoSaving} onChange={(event) => setProductNameDraft(event.target.value)} onBlur={() => keyboard.hide()} /></label><label className="product-info-editor-field"><span>브랜드</span><KeyboardInput className="app-input" value={productBrandDraft} autoComplete="off" spellCheck={false} aria-label="브랜드 수정" disabled={productInfoSaving} onChange={(event) => setProductBrandDraft(event.target.value)} onBlur={() => keyboard.hide()} /></label><label className="product-info-editor-field"><span>분류</span><KeyboardInput className="app-input" value={productCategoryDraft} autoComplete="off" spellCheck={false} aria-label="분류 수정" disabled={productInfoSaving} onChange={(event) => setProductCategoryDraft(event.target.value)} onBlur={() => keyboard.hide()} /></label><p className="product-info-editor-note"><InfoCircledIcon width={14} height={14} />상품명을 바꾸면 기존 상품 출처는 제거되고 변경 이력으로 남아요. 날짜·수량·보관 위치는 유지됩니다.</p><div className="product-info-editor-actions"><button type="button" className="secondary-sheet-button" disabled={productInfoSaving} onClick={() => { keyboard.hide(); setEditProductInfoOpen(false); }}>취소</button><button type="button" className="primary-sheet-button" onPointerDown={(event) => event.preventDefault()} disabled={productInfoSaving || !productNameDraft.trim() || !productBrandDraft.trim() || !productCategoryDraft.trim()} onClick={() => { onUpdateProductInfo(food.id, { name: productNameDraft.trim(), brand: productBrandDraft.trim(), category: productCategoryDraft.trim() }); keyboard.hide(); }}>{productInfoSaving ? "저장 중" : "상품 정보 저장"}</button></div></div> : <button type="button" className="product-info-edit-button" disabled={readOnly} onClick={() => setEditProductInfoOpen(true)}><Pencil1Icon width={15} height={15} /><span><strong>{readOnly ? "상품 정보 수정은 연결 후 가능" : "상품 정보 수정"}</strong><small>{readOnly ? "최근 화면에서는 변경할 수 없어요." : "상품명이 다르면 직접 고칠 수 있어요."}</small></span><ArrowRightIcon width={15} height={15} /></button>}
      <div className={`date-proof-card ${food.dateKind === "estimated_use_first" ? "date-proof-estimated" : ""}`}><div className="proof-icon"><CalendarIcon width={18} height={18} /></div><div><span>{getDateBadge(food)}</span><strong>{food.dateKind === "estimated_use_first" ? food.dateLabel + "까지 먼저 먹기" : food.dateKind === "unknown" ? "날짜 확인 필요" : dateProofDetail}</strong><small>{dateSourceLabel(food.dateSource)}</small></div><button type="button" onClick={onShowGuidance} aria-label="날짜 기준 자세히 보기"><InfoCircledIcon width={16} height={16} /></button></div>
      {dateReviewReason ? <div className="date-review-callout" style={compactPrintedDateRecheck ? { gridTemplateColumns: "auto minmax(0, 1fr) auto", marginBottom: -6 } : undefined} role="status" aria-live="polite" aria-atomic="true"><span className="date-review-icon"><InfoCircledIcon width={16} height={16} /></span><span><strong>조리 전 날짜 확인이 필요해요</strong><small>{dateReviewReason === "날짜 의미 확인" ? "포장일·제조일은 소비기한이 아니에요. 이 날짜를 소비기한으로 덮어쓰지 않고, 소비기한이 보이는 면과 보관 상태를 따로 확인해 주세요." : dateReviewReason === "날짜 확인 필요" ? "표시 날짜를 확인하지 못했어요. 포장지와 보관 상태를 확인한 뒤 조리해 주세요." : "표시 날짜가 오늘이거나 지났어요. 포장지·보관 상태·개봉 여부를 다시 확인해 주세요."}</small>{canRecheckPrintedLabel && dateReviewReason === "날짜 의미 확인" ? <button ref={dateReviewActionRef} className="date-edit-button" type="button" onClick={onOpenLabelReview}><CalendarIcon width={15} height={15} /><span><strong>포장지에서 소비기한 다시 확인</strong><small>라벨을 읽고 새 lot 또는 기존 lot 반영 여부를 선택해요.</small></span><ArrowRightIcon width={15} height={15} /></button> : null}</span>{compactPrintedDateRecheck ? <button ref={dateReviewActionRef} className="date-edit-button date-edit-button-compact" style={{ width: "auto", minWidth: 44, minHeight: 44, gap: 5, padding: "0 10px", justifyContent: "center", whiteSpace: "nowrap" }} type="button" aria-label="포장지에서 날짜 다시 확인" title="포장지에서 날짜 다시 확인" onClick={() => onOpenLabelReview?.()}><CalendarIcon width={17} height={17} /><span style={{ display: "inline", flex: "0 0 auto", color: "var(--atelier-amber)", fontSize: 9, fontWeight: 820 }}>날짜 다시 확인</span></button> : null}</div> : null}
