...
function errorMessage(error: unknown) {
  if (!(error instanceof Error)) return "recipe review 요청에 실패했어요.";
  if (isMealApiWorkspaceConflictError(error)) return MEAL_API_WORKSPACE_CONFLICT_MESSAGE;
  if (isMealApiRecipeCatalogConflictError(error)) return "다른 운영자가 먼저 recipe draft를 변경했어요. 최신 draft를 다시 불러와 내용을 확인해 주세요.";
  if (isMealApiRecipeReviewClaimError(error)) {
    if (error instanceof MealApiError && error.code === "recipe_review_claim_conflict") return "다른 운영자가 이 draft를 검토 중이에요. claim이 해제되거나 만료된 뒤 다시 맡아 주세요.";
    if (error instanceof MealApiError && error.code === "recipe_review_claim_expired") return "이 draft의 review claim이 만료됐어요. 다시 맡은 뒤 수정해 주세요.";
    if (error instanceof MealApiError && error.code === "recipe_review_claim_unavailable") return "pending 상태의 draft만 review를 맡을 수 있어요.";
    return "이 draft를 먼저 명시적으로 맡은 뒤 수정해 주세요.";
  }
  if (isMealApiRecipePublishError(error)) return "현재 계정은 recipe 검토만 가능하고 승인·반려 권한은 없어요.";
  if (isMealApiRecipeReviewPersistenceError(error)) return "recipe review를 저장하지 못했어요. 기존 draft를 유지했어요.";
  if (error instanceof MealApiError && error.status === 503) return "recipe review가 서버에서 비활성화되어 있어요.";
  if (error instanceof MealApiError && error.status === 403) return "recipe_admin 계정 또는 local review token을 확인해 주세요.";
  return "recipe review 요청에 실패했어요. 잠시 후 다시 시도해 주세요.";
}
...
export default function RecipeReviewPanel({ workspaceTransport }: RecipeReviewPanelProps) {
    ...
    try {
      ...
      if (!normalizedToken) {
        ...
        if (auth?.role !== "recipe_admin") {
          setError("recipe_admin 계정으로 로그인하거나 local review token을 입력해 주세요.");
          return;
        }
        if (!auth.user_id) {
          setError("recipe_admin 계정 식별자를 확인할 수 없어요. 다시 로그인해 주세요.");
          return;
        }
      ...
      if (preserveCurrentEditor) {
        ...
        setNotice(nextNotice ?? (response.length ? `${response.length}개의 검토 대기 draft를 불러왔어요.` : "검토 대기 중인 draft가 없어요."));
      }
    } catch (reason) {
  ...
  const importDrafts = async () => {
    ...
    try {
      ...
      const summary = `source에서 ${response.accepted_count}개를 읽었고, 새 draft ${response.persisted_count}개를 추가했어요.`;
  ...
  const release = async () => {
    ...
    try {
      ...
      updateDraftState(response);
      setNotice("review claim을 해제했어요. 다른 운영자가 맡을 수 있습니다.");
    } catch (reason) {
      setError(errorMessage(reason));
    } finally {
      setClaiming(false);
  ...
  const reloadSelectedEditor = () => {
    ...
    if (latest) {
      ...
      setNotice("최신 draft를 불러왔어요. 내용을 확인한 뒤 다시 작업해 주세요.");
      return;
    }
  ...
  return (
    <div className="recipe-review-panel">
      <div className="recipe-review-lead">
        ...
        <div><h3>공개 레시피 검토</h3><p>승인된 recipe만 사용자 planner에 보여요.</p></div>
      </div>
      ...
      <form className="recipe-review-token-form" onSubmit={(event) => { event.preventDefault(); void loadDrafts(); }}>
        <label className="app-input-label" htmlFor="recipe-review-token">운영자 review token (legacy)</label>
        <KeyboardInput id="recipe-review-token" className="app-input" type="password" autoComplete="off" value={reviewToken} onChange={(event) => setReviewToken(event.target.value)} placeholder="서버의 review token" />
        <small className="recipe-review-token-hint">recipe_admin 계정으로 로그인했다면 token 없이도 사용할 수 있어요.</small>
      ...
      {sourceStatus ? <section className="recipe-review-source-status" aria-label="레시피 source 연결 상태">
        <div className="recipe-review-source-status-heading">
          <div><span className={`recipe-review-status-dot ${sourceStatus.configured ? "recipe-review-status-ready" : "recipe-review-status-disabled"}`} /> <strong>{sourceStatus.configured ? "source 준비됨" : "source 비활성"}</strong><small>{sourceStatus.service_id} · {sourceStatus.source_name}</small></div>
      ...
      {sourceStatus?.configured ? <form className="recipe-review-import-form" onSubmit={(event) => { event.preventDefault(); void importDrafts(); }}>
        <div className="recipe-review-import-heading"><span className="recipe-review-section-label">공개 source에서 draft 수집</span><small>가져온 레시피는 자동 공개되지 않고 pending queue에만 들어가요.</small></div>
        ...
        <button className="secondary-sheet-button recipe-review-import-button" type="submit" disabled={loading || importing || sourceLoading}>{importing ? "source를 확인하는 중이에요" : "source에서 draft 가져오기"}<UploadIcon width={16} height={16} /></button>
      ...
      {actorId ? <div className="recipe-review-filter-bar" aria-label="recipe review queue 필터">
      ...
      {drafts.length ? <div className="recipe-review-layout">
        <div className="recipe-review-list" aria-label="검토 대기 recipe draft 목록">
        ...
        {editor ? <div className="recipe-review-editor">
          <div className="recipe-review-source"><strong>{editor.title}</strong><small>{editor.source_name} · {editor.source_revision} · {editor.license}</small><a href={editor.source_url} target="_blank" rel="noreferrer">원본 source 열기</a></div>
          {editorRefreshRequired ? <div className="recipe-review-editor-stale" role="alert"><div><strong>최신 draft 확인 필요</strong><small>다른 운영자가 선택한 draft를 변경했어요. 현재 편집 내용은 보존했으며 최신 상태를 확인한 뒤 계속할 수 있어요.</small></div><button className="recipe-review-claim-button" type="button" disabled={claiming || saving} onClick={reloadSelectedEditor}>최신 draft 불러오기</button></div> : <div className={`recipe-review-claim-bar ${claimIsMine ? "recipe-review-claim-mine" : claimExpired ? "recipe-review-claim-expired" : activeClaim ? "recipe-review-claim-other" : "recipe-review-claim-open"}`} role="status">
            <div><strong>{claimIsMine ? "내가 검토 중" : activeClaim ? "다른 운영자가 검토 중" : claimExpired ? "이전 review claim이 만료됨" : "review 담당자 없음"}</strong><small>{claimIsMine ? `${formatClaimExpiry(editor.claim_expires_at)} · ${canPublish ? "저장·승인·반려를 할 수 있어요." : "저장할 수 있지만 승인·반려 권한은 없어요."}` : activeClaim ? `${editor.claimed_by_email ?? "다른 운영자"} · ${formatClaimExpiry(editor.claim_expires_at)}` : "수정·승인·반려 전에 담당자를 명시적으로 지정해 주세요."}</small></div>
            {claimIsMine ? <button className="recipe-review-claim-button" type="button" disabled={claiming || saving} onClick={() => void release()}>review 맡기 해제</button> : activeClaim ? null : <button className="recipe-review-claim-button" type="button" disabled={claiming || saving} onClick={() => void claim()}>{claiming ? "맡는 중" : "이 draft 맡기"}</button>}
          </div>}
          ...
          <div className="recipe-review-ingredients"><span className="recipe-review-section-label">재료 canonicalization</span>{editor.ingredients.map((ingredient, index) => <div className="recipe-review-ingredient" key={`${editor.id}-${index}`}><div className="recipe-review-raw"><strong>{index + 1}. {ingredient.raw_text}</strong><small>{ingredient.parsed_name ? `parser 후보: ${ingredient.parsed_name} ${ingredient.parsed_amount ?? ""}${ingredient.parsed_unit ?? ""}` : "parser가 확정하지 못한 표현"}</small></div><div className="recipe-review-ingredient-inputs"><KeyboardInput className="app-input" disabled={!canEdit} aria-label={`${index + 1}번 canonical name`} placeholder="canonical name" value={ingredient.canonical_name ?? ""} onChange={(event) => updateIngredient(index, { canonical_name: event.target.value || null })} /><KeyboardInput className="app-input" disabled={!canEdit} aria-label={`${index + 1}번 canonical amount`} type="number" min="0.001" step="0.001" placeholder="수량" value={ingredient.canonical_amount ?? ""} onChange={(event) => updateIngredient(index, { canonical_amount: event.target.value ? Number(event.target.value) : null })} /><KeyboardInput className="app-input" disabled={!canEdit} aria-label={`${index + 1}번 canonical unit`} placeholder="단위" value={ingredient.canonical_unit ?? ""} onChange={(event) => updateIngredient(index, { canonical_unit: event.target.value || null })} /></div><button className={`recipe-review-ingredient-status ${ingredient.review_status === "approved" ? "recipe-review-ingredient-status-approved" : ""}`} disabled={!canEdit} type="button" onClick={() => updateIngredient(index, { review_status: ingredient.review_status === "approved" ? "pending" : "approved" })}>{ingredient.review_status === "approved" ? "검토 완료" : "승인 준비"}</button></div>)}</div>
          {auditEvents.length ? <div className="recipe-review-audit" aria-label="recipe review audit"><span className="recipe-review-section-label">최근 review 기록</span>{auditEvents.slice().reverse().map((event) => <div className="recipe-review-audit-row" key={event.id}><span><strong>{event.action === "imported" ? "source 수집" : event.action === "updated" ? "검토 수정" : event.action === "approved" ? "planner 승인" : event.action === "rejected" ? "반려" : event.action === "claimed" ? "review 담당 지정" : "review 담당 해제"}</strong><small>{event.actor_email ?? event.actor_id} · {new Date(event.occurred_at).toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</small></span><small>snapshot {event.draft_snapshot_hash.slice(0, 10)}…</small></div>)}</div> : null}
          <label className="recipe-review-checkbox"><input type="checkbox" disabled={!canEdit} checked={licenseConfirmed} onChange={(event) => setLicenseConfirmed(event.target.checked)} /><span>source 이용조건·이미지 재사용 권한을 확인했습니다.</span></label>
