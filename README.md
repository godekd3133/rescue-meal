...
- PostgreSQL readiness는 전체 compatibility projection table·핵심 column/index와 `rescue_schema_migrations` `001→026` baseline을 fail-closed로 확인하며, migration body와 ledger row를 locked psycopg transaction으로 함께 확정함 ([current schema gate readback](evidence/postgres-schema-gate-readback-2026-09-08.md))
...
- 2026-09-12 현재 source 재검증에서 auth/recipe 설정·guest provisioning·workspace export rate limit·actor/time audit와 guest preview single-flight까지 포함한 API **512 passed / 8 warnings**, connected **109/109 passed**, fixture/mobile **39 passed + 3 skipped**, native **14 passed**, build **760 modules**를 확인했습니다 ([final current-source readback 2026-09-12](evidence/final-current-source-readback-2026-09-12.md), [export audit readback](evidence/export-audit-readback-2026-09-12.md))
...
- workspace export 성공 요청은 actor·role·safe request ID·UTC 시각을 별도 `026_export_audit.sql` append-only audit row에 기록하며, export JSON에는 이 metadata를 넣지 않습니다. 감사 저장 실패는 `account_export_audit_persistence_unavailable` typed `503`으로 파일 생성 전에 종료하고, audit write는 workspace revision을 증가시키지 않습니다 ([export audit readback](evidence/export-audit-readback-2026-09-12.md))
...
- 320px 홈 queue의 보조 상품 메타는 `7px` 대신 `8px`로 읽기 크기를 보정했으며, queue row·식단 CTA·하단 nav 위치와 320px width는 유지했습니다. focused home **1 passed**.
...
- `GET /api/account/export` — 현재 workspace의 재고·상품 provenance 변경 이력·구매 summary·보관 event·식단·알림 설정 export (secret·OCR 원문·파일명·push endpoint 제외); 성공 생성은 actor/time audit를 별도 기록하고 audit 저장 실패는 파일 없이 typed `503`으로 반환
